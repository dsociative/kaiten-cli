...
/// Where the API base URL came from — shown by `auth status`.
...
pub enum BaseUrlSource {
    ...
    File,
    /// Derived from a domain (`KAITEN_DOMAIN` or `domain` in config.toml)
    Domain,
}
...
pub fn resolve_from(
    ...
    let (base_url, base_url_source) = resolve_base_url(&file, env)?;
    Ok(Resolved {
        base_url,
        base_url_source,
        token,
...
/// KAITEN_BASE_URL → KAITEN_DOMAIN → base_url из файла → domain из файла.
fn resolve_base_url(
    file: &FileConfig,
    env: &HashMap<String, String>,
    ...
    if let Some(domain) = env.get("KAITEN_DOMAIN").filter(|d| !d.is_empty()) {
        return Ok((domain_base_url(domain), BaseUrlSource::Domain));
    }
    if let Some(url) = file.base_url.as_deref().filter(|u| !u.is_empty()) {
...
mod tests {
    ...
    fn env_domain_beats_file_base_url() {
        ...
        let resolved = resolve_from(file, &env).unwrap();
        assert_eq!(resolved.base_url, "https://envdomain.kaiten.ru/api/latest");
        assert!(matches!(resolved.base_url_source, BaseUrlSource::Domain));
    }
